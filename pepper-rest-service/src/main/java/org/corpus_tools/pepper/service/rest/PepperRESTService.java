...
public class PepperRESTService implements PepperServiceImplConstants {
	...
	// logger.debug("Received data on path "+PATH_JOB+"/ (POST):
	// "+System.lineSeparator()+jobDesc);
	// }
	...
	// DataManager.getInstance().put(id);//FIXME THIS IS JUST DEBUG ! move to
	// getStatusReport() where job is started and put it on the dataManager as
	...
	// newTarget =
	// TARGET_ROOT_DIR+stepDesc.getCorpusDesc().getCorpusPath().lastSegment();
	...
	// @Path("job/{id}/{stepNo}")
	// public byte[] getData(@PathParam("id") String jobId, @PathParam("stepNo")
	// Integer stepNo){
	//
	// PepperJob job = pepper.getJob(jobId);
	//
	// URI exportPath=
	// job.getStepDescs().get(stepNo).getCorpusDesc().getCorpusPath();
	//
	...
	// try{
	// logger.info("Providing requested files:
	// "+jobId+File.separator+exportPath);
	// if (!zipFile.exists()){
	// FileOutputStream fos = new FileOutputStream(zipFile);
	// ZipOutputStream zipout = new ZipOutputStream(fos);
	// ZipEntry entry = null;
	// for (File file : FileUtils.listFilesAndDirs(dataDir,

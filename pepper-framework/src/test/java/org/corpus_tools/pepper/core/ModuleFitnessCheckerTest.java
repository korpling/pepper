...
public class ModuleFitnessCheckerTest {
	...
	public void whenModuleIsReadyToRun_thenCorrespondingHealthFeatureShouldBeTrue() {
		...
		assertThat(new ModuleFitnessChecker().checkHealth(module).getFitness(FitnessFeature.IS_READY_TO_RUN)).isTrue();
	}
	...
	public void whenModuleIsNotReadyToRun_thenCorrespondingHealthFeatureShouldBeTrue() {
		...
		assertThat(new ModuleFitnessChecker().checkHealth(module).getFitness(FitnessFeature.IS_READY_TO_RUN)).isFalse();
	}
	...
	public void whenCheckingFitnessFeatureThrowsException_thenCorrespondingFitnessFeatureShouldBeFalse() {
		...
		assertThat(new ModuleFitnessChecker().checkFitness(module).getFitness(FitnessFeature.HAS_NAME)).isFalse();
	}
	...
	public void whenModuleHasName_thenCorrespondingFitnessFeatureShouldBeTrue() {
		...
		assertThat(new ModuleFitnessChecker().checkFitness(module).getFitness(FitnessFeature.HAS_NAME)).isTrue();
	}
	...
	public void whenModuleHasDescription_thenCorrespondingFitnessFeatureShouldBeTrue() {
		...
		assertThat(new ModuleFitnessChecker().checkFitness(module).getFitness(FitnessFeature.HAS_DESCRIPTION)).isTrue();
	}
	...
	public void whenModuleHasSupplierHomepage_thenCorrespondingFitnessFeatureShouldBeTrue() {
		...
		assertThat(new ModuleFitnessChecker().checkFitness(module).getFitness(FitnessFeature.HAS_SUPPLIER_HP)).isTrue();
	}
	...
	public void whenImporterSupportsIsImportable_thenCorrespondingFitnessFeatureShouldBeTrue() {
		...
		assertThat(new ModuleFitnessChecker().checkFitness(module).getFitness(FitnessFeature.IS_IMPORTABLE)).isTrue();
	}
	...
	// @Test
	// public void
	// whenSelfTestAndNoPepperModuleWasSpecified_thenHasSelfTestIsFalse() {
	// final Pepper pepper = mock(Pepper.class);
	// final ModuleFitness fitness = new
	// ModuleFitnessChecker(pepper).selfTest(null, null);
	//
	...
	// @Test
	// public void
	// whenSelfTestAndPepperModuleReturnsEmptyTestDesc_thenHasSelfTestIsFalse()
	// {
	// final Pepper pepper = mock(Pepper.class);
	// final PepperImporter importer = mock(PepperImporter.class);
	// when(importer.getSelfTestDesc()).thenReturn(null);
	// final ModuleFitness fitness = new
	// ModuleFitnessChecker(pepper).selfTest(importer, null);
	//
	...
	// @Test
	// public void
	// whenSelfTestAndModuleIsImporterAndEverythingIsOk_thenAllFeaturesShouldBeTrue()
	// {
	// // GIVEN
	// final SelfTestDesc desc = mock(SelfTestDesc.class);
	...
	// when(desc.compare(any(SaltProject.class),
	// any(SaltProject.class))).thenReturn(true);
	...
	// @Test
	// public void
	// whenSelfTestAndModuleIsImporterAndImportedCorpusIsNotEqualToExpectedOne_thenPassedSelfTestShouldBeFalse()
	// {
	// // GIVEN
	// final SelfTestDesc desc = mock(SelfTestDesc.class);
	// when(desc.getInputCorpusPath()).thenReturn(SAMPLE_INPUT_PATH);
	...
	// when(desc.isValid(Matchers.anyListOf(String.class))).thenReturn(true);
	// when(desc.compare(any(SaltProject.class),
	// any(SaltProject.class))).thenReturn(false);
	// final PepperImporter importer = spy(DoNothingImporter.class);
	// doReturn(desc).when(importer).getSelfTestDesc();
	// when(importer.isImportable(any(URI.class))).thenReturn(1.0);
	// when(importer.getSaltProject()).thenReturn(SampleGenerator.createSaltProject());
	...
	// @Test
	// public void
	// whenSelfTestAndModuleIsManipulatorAndEverythingIsOk_thenFeaturesShouldBeTrue()
	// {
	// // GIVEN
	// final SelfTestDesc desc = mock(SelfTestDesc.class);
	// when(desc.getInputCorpusPath()).thenReturn(SAMPLE_INPUT_PATH);
	...
	// // WHEN
	// final ModuleFitness fitness = new
	// ModuleFitnessChecker(PEPPER).selfTest(manipulator, null);
	// // THEN
	// assertThat(fitness.getFitness(FitnessFeature.HAS_SELFTEST)).isTrue();
	// assertThat(fitness.getFitness(FitnessFeature.HAS_PASSED_SELFTEST)).isFalse();
	// assertThat(fitness.getFitness(FitnessFeature.IS_VALID_SELFTEST_DATA)).isTrue();
	...
	// @Test
	// public void
	// whenSelfTestAndModuleIsExporterAndEverythingIsOk_thenFeaturesShouldBeTrue()
	// {
	// // GIVEN
	// final SelfTestDesc desc = mock(SelfTestDesc.class);
	// when(desc.getInputCorpusPath()).thenReturn(SAMPLE_INPUT_PATH);
	...
	// // WHEN
	// final ModuleFitness fitness = new
	// ModuleFitnessChecker(PEPPER).selfTest(exporter, null);
	// // THEN
	// assertThat(fitness.getFitness(FitnessFeature.HAS_SELFTEST)).isTrue();
	// assertThat(fitness.getFitness(FitnessFeature.HAS_PASSED_SELFTEST)).isFalse();
	// }

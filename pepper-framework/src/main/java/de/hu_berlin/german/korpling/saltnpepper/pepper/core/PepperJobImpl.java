...
/**
 * This class represents a single, but entire conversion process in Pepper, containing the entire workflow
 * like import, manipulation and export.
 * <br/>
 ...
 * all steps, see {@link #getSteps()}. <strong>Note: Do not use this list to add a further step.
 * Use {@link #addStep(PepperModule)} instead.</strong>
 * 
 * <h2>Order of tasks to be done for start of processing</h2>
 * If you do not follow this order in calling, and for instance call task 3 fist, than task 1 and task 2
...
public class PepperJobImpl extends PepperJob{
	 ...
	 * with the list, make a local copy and don't call this method twice.</strong>
	 * @return list of {@link Step} objects. 
	 **/
	...
	/**
	 * Creates a new {@link Step} object containing all values of the passed {@link StepDesc} object and adds it 
	 * to the workflow covered by this {@link PepperJobImpl}. Further tries to resolve the described {@link PepperModule}. 
	 ...
	 * Which means, that:
	 * <ol>
	 * <li>each step of {@link #getImportSteps()} is wired via a initial document bus with the framework (no wiring, but listening on {@link #ID_INTITIAL})</li>
	 * <li>each step of {@link #getManipulationSteps()} is wired with following step in list with a {@link DocumentBus}.<li/>
	 * <li>each step of {@link #getExportSteps()} is wired via a terminal document bus with the framework (no wiring, but listening on {@link #ID_TERMINAL})</li>
	 * </ol>
	 */
	protected synchronized void wire(){
		...
		for (Step importStep: getImportSteps()){
//			DocumentBus initialDocumentBus= new DocumentBus(ID_INTITIAL, importStep.getModuleController().getId());
//			DocumentBus initialDocumentBus= new InitialDocumentBus(ID_INTITIAL, importStep.getModuleController().getId());
...
//		DocumentBus terminalDocumentBus= new TerminalDocumentBus(exportStepIds, ID_TERMINAL);
		...
		{//direct connect importers and exporters with one central bus
	...
	/**
	 * Returns a temporary directory for this job, where all temporary files can be stored.
	 * @return
	 */
	...
	/**
	 * Imports corpus structures of all registered {@link ImportCorpusStructureTest} steps. After calling
	 ...
	 * all following modules will be asked, if they want to influence the order of importing documents. If this is the
	 * case, an order is created and put to all {@link PepperImporter} objects.
	 * <br/>
	 * This method produces as much as {@link SCorpusGraph} objects as {@link Step} given in import step list {@link #getImportSteps()}. 
	 * The position of {@link SCorpusGraph} corresponding to {@link PepperImporter} (importing that graph) in 
	 * {@link SaltProject#getSCorpusGraphs()} is equivalent to position of {@link Step} in list {@link #getImportSteps()}.  
	 */
	protected synchronized void importCorpusStructures(){
		try{
			...
			for (Step importStep: getImportSteps()){
				...
				if (	(getSaltProject().getSCorpusGraphs().size()> numOfImportStep)&&
						(getSaltProject().getSCorpusGraphs().get(numOfImportStep)!= null)){
			...
			for (Step step: getImportSteps()){
				...
				for (SElementId sDocumentId: importOrder){
					...
					// sets flag to determine whether garbage collector should be called after document was send to sleep
...
//					docFile.deleteOnExit();
			...
			else throw new PepperFWException("An exception occured in job '"+getId()+"' while importing the corpus-structure. See nested exception: ",e);
		}
	}
	/**
	 * Returns a list of {@link SElementId}s corresponding to the {@link SDocument} objects contained in the
	 * passed {@link SCorpusGraph} object. If all registered modules, do not make a proposal, the natural one (the one
	 * given by the order of {@link SDocument}s in {@link SCorpusGraph}) is taken. 
	 * <strong>Note: Currently, this method does not a real unification, if more than one proposals are given, the first one is
	 * taken.</strong>
	...
	/** A list of all {@link DocumentControllerImpl} objects corresponding to each {@link SDocument} belonging to this job. **/
	protected List<DocumentController> documentControllers= null;
	/**
	 * Returns a list of all {@link DocumentControllerImpl} objects corresponding to each {@link SDocument} belonging to this job.
	 * @return a list of all {@link DocumentControllerImpl}s
	 */
	protected List<DocumentController> getDocumentControllers(){
	...
	/**
	 * Checks for each {@link PepperModule} in all steps, if it is ready to start, via calling {@link PepperModule#isReadyToStart()}. 
	...
	**/
	 ...
	 * <li>If {@link PepperImporter#importCorpusStructure(de.hu_berlin.german.korpling.saltnpepper.salt.saltCommon.sCorpusStructure.SCorpusGraph)} has not already been called, it will be done.<li>
	 * </ul>
	 */
	public void convert(){ 
	 ...
	 * with the difference, that no {@link PepperExporter} could be defined. Instead, the processed {@link SaltProject} is
	 * the result. 
	 */
...
// ======================================= start: managing number of active documents	
		...
		/** 
		 * Determines the maximal number of {@link SDocument} objects which could be processed 
		 * at the same time
		 **/
		...
		/**
		 * Returns the maximal number of {@link SDocument} objects which could be processed 
		 * at the same time
		...
		/** 
		 * Determines the current number of {@link SDocument} objects which could be processed 
		 * at the same time
		 **/
		private volatile int currNumOfDocuments= 0;
		
		/**
		 * Returns the current number of {@link SDocument} objects which could be processed 
		 * at the same time
		 * @return number of documents
		 */
		...
		/** condition for notifying {@link #getPermissionForProcessDoument()} when a document was released via {@link #releaseDocument()}**/
		private volatile Condition numOfDocsCondition= numOfDocsLock.newCondition();
		/**
		 * Returns true, if a {@link SDocument} or more precisely spoken a {@link SDocumentGraph} 
		 * could be woken up or imported. This is the case, as long as:
		 * <br/> 
		 * {@link #getCurrNumberOfDocuments()} < {@link #getMaxNumberOfDocuments()}.
		 * <br/>
		 * @return true, when #getCurrNumberOfDocuments()} < {@link #getMaxNumberOfDocuments(), false otherwise
		 */
		 ...
		 * ({@link #getCurrNumberOfDocuments()}). 
		 */
		public void releaseDocument(){
			numOfDocsLock.lock();
			try{
				currNumOfDocuments--;
				//TODO not sure, if signal() is correct, or if signalAll() should be used, but I would think, that only one waiter has to be notified 
				numOfDocsCondition.signal();
			}finally{
				numOfDocsLock.unlock();
			}
		}
// ======================================= end: managing number of active documents	
	...
	public void load(URI uri){
		if (uri.isFile()){
	        ...
	        } catch (SAXException e) 
	        {
				...
				else throw new PepperModuleXMLResourceException("Cannot load Pepper workflow description file'"+wdFile+"', because of a nested exception. ",e);
			}
		}else throw new UnsupportedOperationException("Currently Pepper can only load workflow description from local files.");
	}

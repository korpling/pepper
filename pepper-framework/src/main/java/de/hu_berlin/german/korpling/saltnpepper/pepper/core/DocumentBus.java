...
/**
 * This object is an implementation of a threadsafe queue to store {@link DocumentController} objects.
 * Such an object is used to connect multiple {@link ModuleController} objects.
 * An {@link DocumentController} can only be removed from queue, when all output 
 * {@link ModuleController} objects requested the {@link DocumentController} object.
 * <br/>
 * 
 * This object has a list of Ids belonging to {@link ModuleController} objects, which are on the output side of this queue. 
 * All these objects have to request a document, before this queue can get the status finished. The same goes for {@link ModuleController}
 * objects on the input side. All these controllers have to notify this queue to be finished, before the queue can notify the controllers on the
 * output side, that this queue is finished. 
 * 
...
public class DocumentBus 
{
	 ...
	 * @param inputModuleControllerId  identifier of controller, which produces documents for this bus 
	 * @param outputModuleControllerId identifier of controller, which consumes documents for this bus
	 */
	 ...
	 * Creates an object of this type and sets the passes the given input controller and output controller.
	 * @param inputModuleControllerId  identifier of controller, which produces documents for this bus 
	 * @param outputModuleControllerIds list of identifier of controller, which consumes documents for this bus
	 */
	...
	/**
	 * Creates an object of this type and sets the passes the given input controller and output controller.
	 * @param inputModuleControllerIds  list of identifier of controller, which produces documents for this bus 
	 * @param outputModuleControllerId identifier of controller, which consumes documents for this bus
	 */
	...
	/**
	 * Creates an object of this type and sets all passed in- and output {@link ModuleController} objects. 
	 * This is important, because an {@link DocumentController} can only be removed from queue, when all
	 * output {@link ModuleController} objects requested the {@link DocumentController} object.  
	 * @param inputModuleControllerIds  list of identifier of controller, which produces documents for this bus
	 * @param outputModuleControllerIds list of identifier of controller, which consumes documents for this bus
	 */
	...
	/**
	 * Creates an object of this type and passes all output {@link ModuleController} objects. 
	 * This is important, because an {@link DocumentController} can only be removed from queue, when all
	 * output {@link ModuleController} objects requested the {@link DocumentController} object.  
	 * @param outputs
	 */
	...
	{
		if (	(inputModuleControllerIds== null)||
				(inputModuleControllerIds.isEmpty()))
			throw new PepperFWException("Cannot create a document bus with an empty list of input module controllers.");
		if (	(outputModuleControllerIds== null)||
				(outputModuleControllerIds.isEmpty()))
		...
		if (	(getInputControllerIds()!= null)&&
				(getInputControllerIds().size()!= 0)){
			int i= 0;
		...
		str.append(" ==> ");
		if (	(getOutputControllerIds()!= null)&&
				(getOutputControllerIds().size()!= 0)){
			int i= 0;
	...
	/** 
	 * A list of Ids belonging to {@link ModuleController} objects, which are on the output side of this queue. 
	...
	**/
	...
	/**
	 * Returns a list of Ids belonging to {@link ModuleController} objects, which are on the output side of this queue. 
	 * All these objects have to request a document, before this queue can get the status finished.
	...
	/**
	 * A list of Ids belonging to {@link ModuleController} objects on the input side. All these controllers have to notify this queue to be finished, before the queue can notify the controllers on the
	 * output side, that this queue is finished. 
	 */
	protected List<String> inputModuleControllerIds= null;
	/**
	 * Returns a list of Ids belonging to {@link ModuleController} objects on the input side. All these controllers have to notify this queue to be finished, before the queue can notify the controllers on the
	 * output side, that this queue is finished.
	 * @return list of all ids
	 */
	...
	/**
	 * Creates an id for this object. the id consists of the in- and output {@link DocumentController} objects.
	 * @return
	 */
	public String getId(){
		return(id);
	 ...
	 * @param pepperJob {@link PepperJob} object containing this
	 */
	public void setPepperJob(PepperJobImpl pepperJob) {
		this.pepperJob = pepperJob;
	}
	/**
	 * This table is the central management object of the {@link DocumentBus}. This table relates a queue containing document controllers
	 * to a identifier of a {@link ModuleController} object. Key= identifier, value= queue 
	 */
	protected volatile Hashtable<String, ConcurrentLinkedQueue<DocumentController>> documentBus= null; 
	/**
	 * Returns the table, which  is the central management object of the {@link DocumentBus}. This table relates a queue containing document controllers
	 * to a identifier of a {@link ModuleController} object. Key= identifier, value= queue
	 * @return
	 */
	...
	/**
	 * Initializes the table, which  is the central management object of the {@link DocumentBus}. This table relates a queue containing document controllers
	 * to a identifier of a {@link ModuleController} object. Key= identifier, value= queue.
	 * <br/>
	 * Please take care not to call this method, when this object is already started, since this will confuse the started process.
	 * @param outputModuleControllerIds a list of ids of the {@link ModuleController} objects on the output side.
	 */
	...
	/**
	 * This lock is used for creating the condition {@link #waitUntilAllDocumentsArePut} to let the {@link #pop(String)} method wait
	 * until all documents have been put.
	 */
	protected ReentrantLock lock= new ReentrantLock(true);
	/**
	 * Used to let the {@link #pop(String)} method wait
	 * until all documents have been put.
	 */
	 ...
	 * {@link ModuleController} objects on input side, are set to status finish, this queue is finished
	 * and no further documents are passed to {@link ModuleController} objects on output side.  
	 * @param inputControllerId id of controller to be set to finish. 
	 */
	public void finish(String inputControllerId) 
	{
		if (	(inputControllerId== null)||
				(inputControllerId.isEmpty()))
	...
	/**
	 * Returns true, whether all input {@link ModuleController} objects called the {@link #finish(String)} method
	 * and set this object to status finish.
	 ...
	 * @param documentController the {@link DocumentController} object to be added to the queue.
	 */
...
//			}
	...
	/**
	 * Returns a {@link DocumentController} object, which is on first place of the internal queue
	 * (regarding the FIFO principle). When all registered {@link ModuleController} objects
	 * popped the {@link DocumentController} object, it will be removed from internal queue.
	 * <br/>
	 * If the document, which is the one to be taken in queue is in sleep mode, it will be woken up, if 
	 * the {@link PepperJobImpl} gives the permission. Otherwise, this method waits for the permission.
	 * <br/>
	 * Caution, call this method can take some time, since it waits for two conditions:
	 * <ol>
	 * 	<li>a document is waiting in the queue, and the preceding {@link PepperModule} has not finished all documents</li>
	 *  <li>the Pepper framework (more precisely the {@link PepperJobImpl}) gives the permission to wake up the document, if it was in sleep mode<li>
	 * </ol>
	 * 
	 * @param moduleController object requesting the {@link DocumentController} object
	 ...
	 * 
	 * @param outputControllerId object requesting the {@link DocumentController} object
	 ...
	 * <strong>Note, that this String cannot be used for serialization/deserialization purposes.</strong>
	 */
	@Override

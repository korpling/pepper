...
public class PepperModuleImpl implements PepperModule, UncaughtExceptionHandler {
    ...
    /**
     * Creates a {@link PepperModule} object, which is either a {@link MODULE_TYPE#IMPORTER}, a
     * {@link MODULE_TYPE#MANIPULATOR} or a {@link MODULE_TYPE#EXPORTER}. The name of this module is
     * set to "MyModule". <br/>
     * We recommend to use the constructor {@link PepperModuleImpl#PepperModuleImpl(String)} and
     * pass a proper name.
     */
    ...
    /**
     * Creates a {@link PepperModule} object, which is either a {@link MODULE_TYPE#IMPORTER}, a
     * {@link MODULE_TYPE#MANIPULATOR} or a {@link MODULE_TYPE#EXPORTER}. The passed name is set as
     * the modules name.
     */
    ...
    /**
     * Sets the name of this {@link PepperModule}. <strong>Note, that a name can only be set once
     * and not be changed again.</strong>
     * 
    ...
    /**
     * This method is called by OSGi framework and sets the component context, this class is running
     * in. This method scans the given {@link ComponentContext} object for symbolic name and version
     * and initializes its values {@link #symbolicName} and {@link #version} with it. When running
     * this class in OSGi context, you do not have to set both values by hand. With the given
     * architecture, the symbolic name and the bundle version will be given by pom.xml, via
     * MANIFEST.MF and finally read by this method. Also sets the internal field storing the
     * {@link ComponentContext} which can be retrieved via {@link #getComponentContext()}.
     * 
    ...
    protected void activate(ComponentContext componentContext) {
        ...
        if ((componentContext != null) && (componentContext.getBundleContext() != null)
            ...
            this.setVersion(
                    componentContext.getBundleContext().getBundle().getVersion().toString());
        }
    }
    ...
    public boolean isReadyToStart() throws PepperModuleNotReadyException {
        ...
        if (getResources() == null) {
            ...
            if (!resourceFile.exists()) {
                startProblems.add("Given resource file '" + resourceFile.getAbsolutePath()
                        + "' does not exist.");
                retVal = false;
            }
        }
    ...
    /**
     * A threadsafe map of all {@link PepperMapperController} objects which are connected with a
     * started {@link PepperMapper} corresponding to their {@link Identifier}.
     */
    ...
    /**
     * Returns a threadsafe map of all {@link PepperMapperController} objects which are connected
     * with a started {@link PepperMapper} corresponding to their
     * 
     * @return
     */
    ...
    /**
     * Returns a {@link ThreadGroup} where {@link PepperMapper} objects and the corresponding
     * threads are supposed to run in.
     * 
     * @return
     */
    ...
    /**
     * Sets a {@link ThreadGroup} where {@link PepperMapper} objects and the corresponding threads
     * are supposed to run in.
     * 
    ...
    /**
     * an internal flag, to check if {@link #start(Identifier)} has been overridden or not. For
     * downwards compatibility to modules implemented with < pepper 1.1.6
     **/
    private boolean isStartOverridden = true;
    /**
     * A map relating {@link Identifier} belonging to {@link SDocument} objects to their
     * {@link DocumentController} container.
     **/
    private Map<String, DocumentController> documentId2DC = null;

    /**
     * Returns the map relating {@link Identifier} belonging to {@link SDocument} objects to their
     * {@link DocumentController} container.
     * 
     * @return map
     */
    ...
    public void start() throws PepperModuleException {
        if (getSaltProject() == null) {
            throw new PepperFWException(
                    "No salt project was set in module '" + getName() + ", " + getVersion() + "'.");
        }
        // creating new thread group for mapper threads
        mapperThreadGroup = new ThreadGroup(Thread.currentThread().getThreadGroup(),
                this.getName() + "_mapperGroup");
        ...
        for (PepperMapperController controller : controllers) {
            try {
                ...
                throw new PepperFWException("Cannot wait for mapper thread '" + controller + "' in "
                        + this.getName() + " to end. ", e);
            }
        }
        this.end();
        ...
        for (PepperMapperController controller : this.getMapperControllers().values()) {
            if (!alreadyWaitedFor.contains(controller)) {
                try {
                    ...
                    throw new PepperFWException("Cannot wait for mapper thread '" + controller
                            + "' in " + this.getName() + " to end. ", e);
                }
    ...
    /**
     * A list containing all global ids, which have been mapped and for which the pepper fw has been
     * notified, that they are processed.
     **/
    private HashSet<String> mappedIds = null;

    /**
     * Returns a list containing all global ids, which have been mapped and for which the pepper fw
     * has been notified, that they are processed.
     * 
        ...
        if (id.getIdentifiableElement() instanceof SDocument) {
            ...
            if (docController == null) {
                throw new PepperFWException("Error in '" + getName() + "'. Cannot find a "
                        ...
                        + SDocument.class.getSimpleName() + " '" + SaltUtil.getGlobalId(id)
                        + "' to pass status '" + result
                        + "'. Controllers are listed for the following Identifier objects: "
                        + getDocumentId2DC() + ". ");
            }
            if (DOCUMENT_STATUS.DELETED.equals(result)) {
                ...
                logger.error("Cannot map '" + SaltUtil.getGlobalId(id) + "' with module '"
                        + this.getName() + "', because of a mapping result was '"
                ...
                throw new PepperModuleException(this,
                        "Cannot notify pepper framework for process of Identifier '" + id.getId()
                                ...
                                + DOCUMENT_STATUS.COMPLETED + "', '" + DOCUMENT_STATUS.FAILED
                                + "' and '" + DOCUMENT_STATUS.DELETED + "' is permitted.");
        }
    }
    ...
    /**
     * This method is called by method {@link #start()}, if the method was not overridden by the
     * current class. If this is not the case, this method will be called for every document which
     * has to be processed.
     * 
        ...
        for (SCorpusGraph sCorpusGraph : corpGraphs) {
            if (sCorpusGraph != null) {
                if (MODULE_TYPE.IMPORTER.equals(getModuleType())) {
                    ...
                    if ((sCorpusGraph.getIdentifier() != null)
                            && (((PepperImporter) this).getCorpusGraph() != null)
        ...
        if ((sElementId != null) && (sElementId.getIdentifiableElement() != null)
            ...
            if (this instanceof PepperImporter) {
                if (mapper.getResourceURI() == null) {
                    URI resource =
                            ((PepperImporter) this).getIdentifier2ResourceTable().get(sElementId);
                    mapper.setResourceURI(resource);
                }
            }

            if ((sElementId.getIdentifiableElement() instanceof SDocument)
                    && (mapper.getDocument() == null)) {
    ...
    public PepperMapper createPepperMapper(Identifier sElementId) {
        throw new NotInitializedException(
                "Cannot start mapping, because the method createPepperMapper() of module '"
     ...
     * necessary because of {@link ConcurrentModificationException}, if any other modules adds or
     * removes a corpus object in the meantime)
     */
    private Collection<SCorpus> corporaToEnd = null;

    /**
     * Calls method {@link #start(Identifier)} for every root {@link SCorpus} of {@link SaltProject}
     * object.
     */
    @Override
    public void end() throws PepperModuleException {
        logger.trace("[{}] start processing corpus structure (manipulating or exporting). ",
                getName());
        if (getSaltProject() == null) {
        ...
        if (getSaltProject().getCorpusGraphs() == null) {
            throw new PepperModuleException(this,
                    "Error in method end() corpus graphs of salt project were empty.");
        }
    ...
    /**
     * Method catches Uncaught exceptions thrown by {@link PepperMapperImpl} while running as
     * Thread. This method just logs the exception. It is assumed, that the {@link PepperMapperImpl}
    ...
    public Double getProgress() {
        Collection<PepperMapperController> controllers =
                Collections.synchronizedCollection(this.getMapperControllers().values());
        Double progress = 0d;
     ...
     * <strong>Note: This representation cannot be used for serialization/ deserialization
     * purposes.</strong>
     */
    @Override
